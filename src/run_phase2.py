...
def load_data_from_windows(window_dir, window_size):
    """Loads ONLY the flux data from window files."""
    all_flux_data = []
    window_files = glob.glob(os.path.join(window_dir, "*.csv"))

    if not window_files:
        raise FileNotFoundError(f"No window files found in {window_dir}")
        
    for f in window_files:
        df = pd.read_csv(f)
        
        # *** DEFINITIVE FIX ***
        # ONLY use the flux column, ignore the massive TIC ID numbers
        flux = df['flux'].values
        ...
        # Ensure flux is in a reasonable range (0.5 to 1.5 for normalized stellar flux)
    ...
    logger.info(f"Loaded flux data - Shape: {flux_tensor.shape}, Min: {flux_tensor.min():.4f}, Max: {flux_tensor.max():.4f}")
...
def run_phase2(data_dir="data", num_epochs=50, num_synthetic_samples=1000, batch_size=32):
    ...
    logger.info(f"Training generator for {num_epochs} epochs...")
    ...
    with torch.no_grad():
        for i in range(num_synthetic_samples):
            z = torch.randn(1, model.noise_dim).to(device)
            synthetic_flux = model(z).squeeze().cpu().numpy()
    ...
    phase2_summary = {
        ...
        "status": "Success",
        "model_used": "Simple Generator",
        "synthetic_samples_generated": num_synthetic_samples
    }
    
    with open(os.path.join(results_dir, "phase2_results.json"), "w") as f:
        json.dump(phase2_summary, f, indent=2)
    summary = {'timestamp': datetime.now().isoformat(), 'model_used': 'Simple Generator'}
    return summary
...
if __name__ == "__main__":
    ...
    run_phase2(data_dir=args.data_dir, num_epochs=args.epochs, num_synthetic_samples=args.samples)
